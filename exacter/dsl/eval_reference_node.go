...
func (n *EvalReferenceNode) EvalString(scope *Scope, executionState ExecutionState) (string, error) {
	...
	if refType == ast.TList {
		return strings.Join(refValue.([]string), ","), nil
	}
...
func (n *EvalReferenceNode) Eval(scope *Scope, _ ExecutionState) (interface{}, error) {
	return n.getReferenceValue(scope)
}
