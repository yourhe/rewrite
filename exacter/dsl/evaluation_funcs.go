...
var evaluationFuncs = map[operationKey]*evaluationFnInfo{
	...
	operationKey{operator: ast.TokenMatch, leftType: ast.TString, rightType: ast.TString}: {
		f: func(scope *Scope, executionState ExecutionState, leftNode, rightNode NodeEvaluator) (resultContainer, *ErrSide) {
			var left string
			var right string
			var err error
	...
	operationKey{operator: ast.TokenMatch, leftType: ast.TList, rightType: ast.TString}: {
		f: func(scope *Scope, executionState ExecutionState, leftNode, rightNode NodeEvaluator) (resultContainer, *ErrSide) {
			var left string
			var right string
			var err error
			...
			return resultContainer{BoolValue: strings.Index(left, right) > -1, IsBoolValue: true}, nil
